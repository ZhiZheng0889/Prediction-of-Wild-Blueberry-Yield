...
# Train base models using caret
set.seed(42)
rf_model1 <- train(yield ~ ., data = train_set, method = "rf", trControl = trainControl(method = "cv", number = 5))
rf_model2 <- train(yield ~ ., data = train_set, method = "rf", trControl = trainControl(method = "cv", number = 5))

# Create a caretList of base models
base_models <- caretList(rf_model1 = rf_model1, rf_model2 = rf_model2)

# Train the Stacking model
set.seed(42)
stack_control <- trainControl(method = "cv", number = 5, savePredictions = TRUE, verboseIter = FALSE)
stacking_model <- caretStack(base_models, method = "lm", trControl = stack_control)

# Make predictions using the stacking model
predictions <- predict(stacking_model, newdata = test_data %>% dplyr::select(-id))

# Create a dataframe with 'id' and 'yield' columns
submission <- data.frame(id = test_data$id, yield = predictions)
